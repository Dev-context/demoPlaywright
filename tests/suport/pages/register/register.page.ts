...
export default class Register extends Baseclass {
	...
	async selectSomeLanguages(languages: string[]) {
		...
		await Actions.multiSelect(await this.languages.locator("li:has(a)").all(), languages)
	...
	async selectCountry(option: string) {
		const presentation = this.page.locator("span.select2-selection__arrow").filter({ has: this.page.getByRole("presentation") })
